...
When an upgrade needs an Orchid SQLite schema migration, Orchid creates a consistent, one-time sibling backup before changing an existing database. For the current schema version 3, `/data/orchid.db` produces `/data/orchid.pre-schema-v3.bak`. The migration is additive, but downgrading to a binary that cannot read the newer schema is unsupported: stop Orchid and restore that pre-migration backup instead. Do not delete the backup until the upgraded release is accepted.
